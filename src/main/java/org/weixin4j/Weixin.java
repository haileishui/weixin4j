...
public class Weixin extends WeixinSupport implements java.io.Serializable {
    ...
    public void sendTemplate(JSONObject data) throws WeixinException {  
    	//必须先调用检查登录方法
        checkLogin();
    	//创建请求对象
        HttpsClient http = new HttpsClient();
        Response res = http.post("https://api.weixin.qq.com/cgi-bin/message/template/send?access_token="+this.oauthToken.getAccess_token(),data);
        //根据请求结果判定，是否验证成功
        JSONObject jsonObj = res.asJSONObject();
        if (jsonObj != null) {
            if (Configuration.isDebug()) {
